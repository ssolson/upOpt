...
def getCollections():
    ...
    """
    response = requests.get("https://api.upland.me/collections")
...
def defineDV(
    yield_per_hour, collectionID, yieldBoost, propID, cityID, streetID, address
):
    """
    Defines DV Vars
...
def kingOfTheStreet(properties, NMaxStreets=2, NPropertiesMax=4):
    ...
    for streetID, propIDs in kingOfStreetIDs.items():
        for propID in propIDs:
            ...
            kingDict[decisionVariable] = defineDV(
                yield_per_hour,
                collectionID,
                yieldBoost,
                propID,
    ...
    for cityID in kingData.cityID.unique():
        ...
        if len(cityData) > NMaxStreets:
            ...
            for streetID in cityData.streetID.unique():
                streetYield[streetID] = cityData[
                    cityData["streetID"] == streetID
                ].yield_per_hour.sum()
...
def collectionsDict(properties, allCollections, NKeep=30):
    ...
    for propID, details in properties.iterrows():
        ...
        try:
            ...
            dvDict[decisionVariable] = defineDV(
                yield_per_hour,
                collectionID,
                collectionBoost,
                propID,
    ...
    if NNewbie > NKeep:
        dvData["yield_per_hour"] = pd.to_numeric(dvData.yield_per_hour)
        rmId = (
            dvData[dvData.collectionID == 7]
            .nsmallest(columns="yield_per_hour", n=NNewbie - NKeep)
            .index
        )
        dvData.drop(index=rmId, inplace=True)
    ...
    if NSFian > (NKeep + 30):
        dvData["yield_per_hour"] = pd.to_numeric(dvData.yield_per_hour)
        rmId = (
            dvData[dvData.collectionID == 11]
            .nsmallest(columns="yield_per_hour", n=NSFian - (NKeep + 30))
            .index
        )
        dvData.drop(index=rmId, inplace=True)
...
def write_solution(username, solution):
    ...
    with open(f"{username}.txt", "w") as f:
        ...
        for collection in solution["collections"]:
            monthlyBoost = collection["monthly_earnings"]
            collectionName = collection["name"]
            N = collection["number_needed"]

            f.write("=======================================================\n")
            f.write(
                f"{collectionName}({collection['id']}): [{N} Properties: {monthlyBoost.round(0)} UPX/month]\n"
            )
            f.write("=======================================================\n")
            i = 0
            for prop in collection["properties"]:
                ...
                try:
                    ...
                    f.write(
                        f"{i+1}. {prop['full_address']}    (Mint: {mintPrice/1000} k )\n"
                    )
                else:
                    if active:
                        status = " [Active] "
                        ...
                        status = " [Inactive] "
                    f.write(
                        f"{i+1}.{status}{prop['full_address']}(Mint: {mintPrice/1000} k )\n"
                    )
                i += 1
            f.write("\n")
        ...
        for propID in props:
            ...
            if len(address.values) > 0:
                ...
                if solutionInActive:
                    ...
                    if boostMatch:
                        for coll in optimized["collections"]:
